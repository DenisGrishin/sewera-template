...
*/
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function initSliders() {
      bildSliders();
      // слайдер септиков, условия чтоб слайдер работал на мобилке
      if (width < 767.98) {
  ...
  function refreshDateTable() {
    if (document.querySelector('.spoller-services_date-price span')) {
      const date = new Date();
      ...
      document.querySelector('.spoller-services_date-price span').innerHTML =
        `Цена обновлена на: ${dayCurrent <= 9 ? '0' + dayCurrent : dayCurrent}.${monthCurrent <= 9 ? '0' + monthCurrent : monthCurrent}.${yearCurrent}`;
    }
  }
